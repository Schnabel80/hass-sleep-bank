...
from custom_components.sleep_bank.const import DOMAIN
from custom_components.sleep_bank.models import SleepNight

if TYPE_CHECKING:
    from homeassistant.core import HomeAssistant

EXPECTED_SENSORS = {
    "sensor.sleep_bank_acute_sleep_debt",
    "sensor.sleep_bank_chronic_deficit_per_night",
    "sensor.sleep_bank_nights_to_recovery",
    "sensor.sleep_bank_sleep_regularity_index",
    "sensor.sleep_bank_social_jetlag",
    "sensor.sleep_bank_readiness",
    "sensor.sleep_bank_data_coverage",
}
...
async def test_entitaeten_werden_angelegt(
    ...
    assert "binary_sensor.sleep_bank_sleep_debt_warning" in existing
    assert "button.sleep_bank_recalculate" in existing
    assert "number.sleep_bank_sleep_need" in existing
...
async def test_unzureichende_datenlage_wird_gemeldet(
    ...
    await _setup(hass, config_entry)
    state = hass.states.get("binary_sensor.sleep_bank_insufficient_data")
    assert state is not None
